...
namespace tests.Specs
{
    ...
    {
        [Fact]
        public async Task Login_InteractivePortalLogin()
        {
            // Arrange
            ...
            var portalInteractiveLoginMock = portalRepositoryMock.As<IRepositoryInteractiveLogin>();
            portalRepositoryMock.Setup(x => x.RepositoryType).Returns(CmfAuthConstants.PortalRepositoryType);

            portalInteractiveLoginMock.Setup(x => x.InteractiveLogin())
                .Returns(Task.FromResult<ICredential>(mockCredential));

            authStoreMock.Setup(x => x.GetRepositoryType(CmfAuthConstants.PortalRepositoryType)).Returns(portalRepositoryMock.Object);
            ...
            // Assert
            portalInteractiveLoginMock.Verify(x => x.InteractiveLogin(), Times.Once);
        ...
        public async Task Login_NPMExplicitAuthType()
        {
            ...
            authStoreMock.Setup(x => x.GetRepositoryType(It.IsAny<string>())).Returns(npmRepositoryMock.Object);
            ...
            authStoreMock.Verify(x => x.Save(
                It.Is<IList<ICredential>>(cred => cred.Count == 1 && cred[0] is BearerCredential && ((BearerCredential)cred[0]).Token == token
                                                                                                 && ((BearerCredential)cred[0]).RepositoryType == CmfAuthConstants.NPMRepositoryType
        ...
        public async Task Login_NuGetBasicAuth()
        {
            ...
            authStoreMock.Setup(x => x.GetRepositoryType(It.IsAny<string>())).Returns(nugetRepositoryMock.Object);
            ...
            authStoreMock.Verify(x => x.Save(
                It.Is<IList<ICredential>>(cred => cred.Count == 1 && cred[0] is BasicCredential && ((BasicCredential)cred[0]).Username == username
                                                                                                ...
                                                                                                && ((BasicCredential)cred[0]).RepositoryType == CmfAuthConstants.NuGetRepositoryType
        ...
        public async Task Login_Sync()
        {
            ...
            {
                Repositories = new Dictionary<string, CmfAuthFileRepositoryType>()
                {
                    { "npm", new CmfAuthFileRepositoryType { Credentials = [npmCred1, npmCred2] } },
                    { "nuget", new CmfAuthFileRepositoryType { Credentials = [nugetCred1] } }
                }
            };
            ...
            authStoreMock.SetupSequence(x => x.GetRepositoryType(It.IsAny<string>()))
