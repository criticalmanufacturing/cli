...
public class RepositoryCredentials
{
    ...
    public void RepositoryAuthStore_GetBearerEnvVarCredentials()
    {
        ...
        try
        {
            ...
            {
                { "NPM_REPOURL__AUTH_TYPE", "bearer" },
                { "NPM_REPOURL__TOKEN", "a.b.c" },
            });

            NPMRepositoryMock.Setup(x => x.GetEnvironmentVariablePrefix(It.IsAny<string>())).Returns("NPM_REPOURL");
    ...
    public void RepositoryAuthStore_GetBasicEnvVarCredentials()
    {
        ...
        try
        {
            ...
            environment = new MockEnvironment(new Dictionary<string, string>()
            {
                { "CIFS_REPOURL__AUTH_TYPE", "BASIC" },
                { "CIFS_REPOURL__DOMAIN", "CMF" },
                { "CIFS_REPOURL__USERNAME", "user" },
                { "CIFS_REPOURL__PASSWORD", "querty" },
            });

            CIFSRepositoryMock.Setup(x => x.GetEnvironmentVariablePrefix(It.IsAny<string>())).Returns("CIFS_REPOURL");
    ...
    public void RepositoryAuthStore_GetEnvVarCredentials_WrongAuthType()
    {
        ...
        try
        {
            ...
            environment = new MockEnvironment(new Dictionary<string, string>()
            {
                { "DOCKER_REPOURL__AUTH_TYPE", "bearer" },
                { "DOCKER_REPOURL__TOKEN", "a.b.c" },
            });

            DockerRepositoryMock.Setup(x => x.GetEnvironmentVariablePrefix(It.IsAny<string>())).Returns("DOCKER_REPOURL");
    ...
    [Theory]
    [InlineData("http://registry.npmjs.org/", "npm_registry_npmjs_org")]
    [InlineData("http://custom.registry.com/npm-group/", "npm_custom_registry_com_npm_group")]
    ...
    [Theory]
    [InlineData("https://api.nuget.org/v3/index.json", "nuget_api_nuget_org_v3_index_json")]
    [InlineData("https://custom.io/repository/nuget/index.json", "nuget_custom_io_repository_nuget_index_json")]
    ...
    [Theory]
    [InlineData(@"\\server.com", "cifs_server_com")]
    [InlineData(@"\\server.com\share", "cifs_server_com_share")]
    [InlineData(@"\\server.com\share\sub\folder\path", "cifs_server_com_share")]
