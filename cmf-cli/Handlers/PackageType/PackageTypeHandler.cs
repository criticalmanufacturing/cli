...
namespace Cmf.Common.Cli.Handlers
{
    ...
    public abstract class PackageTypeHandler : IPackageTypeHandler
    {
        ...
        public virtual void RestoreDependencies(Uri[] repoUris)
        {
            ...
            var thisPackage = this.CmfPackage.LoadDependencies(repoUris, true);
            Log.Progress("Finished", true);
            if (thisPackage.Dependencies == null)
            {
            ...
            var allDependencies = thisPackage.Dependencies.Flatten(
            ...
            foreach (var dependency in foundDependencies)
            {
                ...
                {
                    var zipFile = System.IO.Compression.ZipFile.Open(dependency.CmfPackage.Uri.LocalPath, ZipArchiveMode.Read);
