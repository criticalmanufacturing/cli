    ...
    public class IoTCommand : LayerTemplateCommand
    {
        ...
        public void Execute(IDirectoryInfo workingDir, string version, string htmlPackageLocation, bool isAngularPackage)
        {
            if (ExecutionContext.Instance.ProjectConfig.MESVersion.Major > 9)
            {
                ...
                // only introduced in 10.2.0
                else if (ExecutionContext.Instance.ProjectConfig.MESVersion.Major > 10 || (ExecutionContext.Instance.ProjectConfig.MESVersion.Major == 10 && ExecutionContext.Instance.ProjectConfig.MESVersion.Minor >= 2))
                {
                ...
                {
                    throw new CliException("For versions upper than 9 and less than 10.2, please use the angular customization package with flag isAngularPackage as true");
                }
            }
            else
            {
