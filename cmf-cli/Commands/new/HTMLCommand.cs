...
namespace Cmf.Common.Cli.Commands.New
{
    ...
    public class HTMLCommand : UILayerTemplateCommand
    {
        ...
        public void Execute(IDirectoryInfo workingDir, string version, IFileInfo htmlPackage)
        {
            ...
            var htmlDevTasksConfigPath = ExecutionContext.Instance.FileSystem.Path.GetTempFileName();
...
$@"{{
    ""answers"": {{
        ...
        ""channel"": ""release-{mesVersion?.Replace(".", "")}""
            ...
            ExecutionContext.Instance.FileSystem.File.WriteAllText(htmlDevTasksConfigPath, htmlDevTasksConfigJson);
            var htmlWebAppConfigPath = ExecutionContext.Instance.FileSystem.Path.GetTempFileName();
...
@"{
    ""answers"": {
        ...
        ""basePackage"": ""@criticalmanufacturing/mes-ui-web""
            ...
            ExecutionContext.Instance.FileSystem.File.WriteAllText(htmlWebAppConfigPath, htmlWebAppConfigJson);
            ...
            var configJsonPath = ExecutionContext.Instance.FileSystem.Path.Join(pkgFolder.FullName, "apps", 
                ExecutionContext.Instance.FileSystem.Path.Join("customization.web", "config.json"));
            ...
            var webAppPath = ExecutionContext.Instance.FileSystem.Path.Join(pkgFolder.FullName, 
                "apps", "customization.web");
            var webAppPkgJsonPath = ExecutionContext.Instance.FileSystem.Path.Join(webAppPath, "package.json");
            ...
            var relativePathToRoot =
                ...
                ExecutionContext.Instance.FileSystem.Path.GetRelativePath(
                    ...
                    //);
                    .Replace("\\", "/");
            var webAppPkgJsonStr= ExecutionContext.Instance.FileSystem.File.ReadAllText(webAppPkgJsonPath);
            ...
            ExecutionContext.Instance.FileSystem.File.WriteAllText(webAppPkgJsonPath, webAppPkgJsonStr);
            
            // remove package locks
            Log.Verbose("Generating accurate package-locks for future installs. This will take a while...");
            var rootPkgLockJsonPath = ExecutionContext.Instance.FileSystem.Path.Join(pkgFolder.FullName, "package-lock.json");
            ExecutionContext.Instance.FileSystem.File.Delete(rootPkgLockJsonPath);
            var webAppPkgLockJsonPath = ExecutionContext.Instance.FileSystem.Path.Join(
                ExecutionContext.Instance.FileSystem.Path.Join(pkgFolder.FullName, "apps", "customization.web"), "package-lock.json");
            ExecutionContext.Instance.FileSystem.File.Delete(webAppPkgLockJsonPath);
            // gulp install --update
            (new GulpCommand()
            {
                WorkingDirectory = pkgFolder,
