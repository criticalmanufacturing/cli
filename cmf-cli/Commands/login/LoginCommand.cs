    ...
    public class LoginCommand : BaseCommand
    {
        ...
        public override void Configure(Command cmd)
        {
            ...
            cmd.AddOption(new Option<bool>(
                ...
                description: "If true, the credentials are stord on the cmfauth.json file, but are not applied to the credentials file of the tool (NPM, NuGet, Docker, etc...)"
            ));
            cmd.AddOption(new Option<bool>(
            ...
            cmd.Handler = CommandHandler
                .Create((string repositoryType, string repository, AuthType? authType, string token, string username, string password, string domain, string key, bool storeOnly, bool noPrompt) =>
                {
                    return Execute(repositoryType, repository, authType, token, username, password, domain, key, storeOnly, noPrompt);
                });
        }
        ...
        /// Execute the command
        /// </summary>
        internal async Task Execute(string repositoryType, string repository, AuthType? authType, string token, string username, string password, string domain, string key, bool storeOnly, bool noPrompt)
        {
            ...
            if (string.Equals(repositoryType, CmfAuthConstants.PortalRepositoryType, StringComparison.InvariantCultureIgnoreCase) &&
            ...
            var repositoryCredentials = authStore.GetRepositoryType(repositoryType);

            if (repositoryCredentials is IRepositoryInteractiveLogin interactiveRepo
                && authType == null && username == null && password == null && token == null)
            {
                credentials = await interactiveRepo.InteractiveLogin();
            }
            else
            {
