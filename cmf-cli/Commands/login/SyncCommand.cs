...
{
    /// <summary>
    /// Synchronizes the credentials that are stored on the cmfauth.json file into the respective tools' configuration files
    /// </summary>
    [CmfCommand("sync", Id = "login_sync", ParentId = "login", Description = "Sync credentials from the cmfauth.json file into each specific tool (npm, nuget, docker, etc...) configuration files.")]
    public class SyncCommand : BaseCommand
    {
        ...
        public override void Configure(Command cmd)
        {
            ...
            cmd.Handler = CommandHandler
                .Create((string repositoryType, string repository) =>
                {
                    return Execute(repositoryType, repository);
                });
        }
        ...
        /// Execute the command
        /// </summary>
        internal async Task Execute(string repositoryType, string repository)
        {
            ...
            string[] resolvedRepositoryTypes;

            if (string.IsNullOrEmpty(repositoryType))
            {
                Log.Debug($"No repository type provided, will use all repositories defined in our cmfauth.json file");
                resolvedRepositoryTypes = authFile.Repositories.Keys.ToArray();
            }
            else
            {
                resolvedRepositoryTypes = [authStore.GetRepositoryType(repositoryType)?.RepositoryType];
            }
            ...
            foreach (var repoType in resolvedRepositoryTypes)
            {
                ...
                {
                    Log.Warning($"File cmfauth.json does not contain any credentials for repository {repoType}");
                    continue;
                }
