    ...
    public abstract class TemplateCommand : BaseCommand
    {
        ...
        protected async Task ExecuteTemplateAsync(string templateName, IReadOnlyCollection<string> args)
        {
            ...
            foreach (var list in argList)
            {
                ...
                {
                    case 1:
                        parameters[list[0].Replace("--", "")] = null;
                        Log.Debug($"Adding template param with key {list[0].Replace("--", "")}");
                        break;
                    case 2:
                        parameters[list[0].Replace("--", "")] = list[1];
            ...
            var outputPath = parameters.TryGetValue("output", out string? value) && !string.IsNullOrEmpty(value) ? value : (ExecutionContext.Instance ?? throw new InvalidOperationException("ExecutionContext not initialized")).FileSystem.Directory.GetCurrentDirectory();
        ...
        internal class CmfCliPackageProviderFactory : ITemplatePackageProviderFactory
        {
            ...
            private class CmfCliPackageProvider : ITemplatePackageProvider, IIdentifiedComponent
            {
                ...
                public Task<IReadOnlyList<ITemplatePackage>> GetAllTemplatePackagesAsync(CancellationToken cancellationToken)
                {
                    ...
                    var executionContext = ExecutionContext.Instance ?? throw new InvalidOperationException("ExecutionContext not initialized");
                    string? assemblyLocation = executionContext.FileSystem.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);

                    IEnumerable<string> expandedPaths = InstallRequestPathResolution.ExpandMaskedPath(executionContext.FileSystem.Path.Join(assemblyLocation, "resources", "template_feed"), settings);
        ...
        protected IEnumerable<string> ParseConfigFile(IFileInfo configFile)
        {
            ...
            if (configJson != null)
            {
                ...
                var tenantValue = configJson["Product.Tenant.Name"]?.Value ?? configJson["TENANT_NAME"]?.Value;
                if (tenantValue != null)
                {
                    args.AddRange(new string[] { "--Tenant", tenantValue });
                }
