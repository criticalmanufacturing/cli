...
namespace Cmf.CLI.Core.Services
{
    public class RepositoryAuthStore : IRepositoryAuthStore
    {
        ...
        protected ICredential GetCredentialsFor(IRepositoryCredentials repositoryType, CmfAuthFile authFile, string repository, bool ignoreEnvVars = false)
        {
            if (!ignoreEnvVars)
            {
                ICredential credentials = GetEnvironmentCredentialsFor(repositoryType, repository);
        ...
        public IRepositoryCredentials GetRepositoryType(string repositoryType)
        {
            var allRepositoryCredentials = ExecutionContext.ServiceProvider.GetServices<IRepositoryCredentials>();

            var repositoryCredentials = allRepositoryCredentials
                .FirstOrDefault(repo => string.Equals(repo.RepositoryType, repositoryType, StringComparison.InvariantCultureIgnoreCase));
        ...
        public ICredential GetEnvironmentCredentialsFor(string repositoryType, string repository)
        {
        ...
        public ICredential GetCredentialsFor(string repositoryType, CmfAuthFile authFile, string repository, bool ignoreEnvVars = false)
        {
        ...
        public async Task<CmfAuthFile> Load()
        {
            try
            {
                ...
                {
                    authFile.Repositories = new Dictionary<string, CmfAuthFileRepositoryType>();
                }
        ...
        public async Task Save(IList<ICredential> credentials, bool sync = true)
        {
            ...
            // Save them in the cmfauth.json file
            ...
            foreach (var (repoType, repoCredentials) in credentialsByRepo)
            {
                if (repoType is IRepositoryDerivedCredentials repoDerived)
                {
                    derivedCredentials.AddRange(repoDerived.GetDerivedCredentials(repoCredentials));
                }
            }
        ...
        public static RepositoryAuthStore FromEnvironmentConfig(IFileSystem fileSystem)
        {
            ...
            if (string.IsNullOrEmpty(authFile))
            {
                ...
                authFile = fileSystem.Path.Join(appData, "cmf", "cmfauth.json");
            }
