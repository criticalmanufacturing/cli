...
public class CmfPackageV1 : IEquatable<CmfPackageV1>
{
    ...
    public CmfPackageV1(string name, string packageId, string version, string description, PackageType packageType,
            ...
            : this(name, packageId, version, description, packageType,
                       targetDirectory, targetLayer, isInstallable, isUniqueInstall, keywords,
                       isToSetDefaultSteps, dependencies, steps,
                       contentToPack, xmlInjection, waitForIntegrationEntries, testPackages, keepDefaultDependencies: false)
    { }

    public CmfPackageV1(string name, string packageId, string version, string description, PackageType packageType,
                      string targetDirectory, string targetLayer, bool? isInstallable, bool? isUniqueInstall, string keywords,
                      bool? isToSetDefaultSteps, DependencyCollection dependencies, List<Step> steps,
                      List<ContentToPack> contentToPack, List<string> xmlInjection, bool? waitForIntegrationEntries, DependencyCollection testPackages = null, bool keepDefaultDependencies = false) : this()
    {
        if (dependencies != null)
        {
            if (!keepDefaultDependencies)
            {
